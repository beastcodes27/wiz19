...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!verify_csrf_token($_POST['csrf_token'] ?? '')) {
        ...
        if (empty($subject) || empty($category) || empty($msg)) {
            $message = "Please fill in all required fields.";
            $messageType = "danger";
        } else {
            // Combine category and priority into subject for admin context
            $full_subject = "[$category] [$priority] $subject";
            
            $stmt = $pdo->prepare("INSERT INTO support_tickets (user_id, subject, message, status) VALUES (?, ?, ?, 'open')");
            if ($stmt->execute([$user_id, $full_subject, $msg])) {
...
<head>
  ...
  <title><?= htmlspecialchars($platform_name) ?> - Support</title>
        ...
        <div class="container">
            <div class="app-page-head d-flex flex-wrap gap-3 align-items-center justify-content-between">
                <div class="clearfix">
                    ...
                    <p class="text-muted mb-0">Need help? Submit a ticket and we'll get back to you shortly.</p>
                </div>
            </div>
            ...
            <div class="row">
                <div class="col-lg-5 mb-4">
                    <div class="card border-0 shadow-sm">
                        <div class="card-header bg-transparent border-bottom py-3">
                            <h5 class="mb-0 fw-bold"><i class="fa-solid fa-headset me-2 text-primary"></i> New Support Ticket</h5>
                        </div>
                        <div class="card-body pt-4">
                            <form action="" method="POST">
                                ...
                                <div class="mb-3">
                                    <label class="form-label fw-semibold text-muted">Category <span class="text-danger">*</span></label>
                                    <select name="category" class="form-select" required>
                                        <option value="">Select a category</option>
                                        <option value="payment">Payment Issue</option>
                                        <option value="upload">Video Upload Problem</option>
                                ...
                                <div class="mb-3">
                                    ...
                                    <select name="priority" class="form-select">
                                        ...
                                        <option value="medium" selected>Medium - Something isn't working</option>
                                        <option value="high">High - Urgent / Money involved</option>
                                    </select>
                                </div>
                                <div class="mb-4">
                                    <label class="form-label fw-semibold text-muted">Message <span class="text-danger">*</span></label>
                                    <textarea name="message" class="form-control" rows="5" placeholder="Describe your issue in detail. Include any transaction IDs, video IDs, or screenshots if applicable." required></textarea>
                                </div>
                                <button type="submit" class="btn btn-primary w-100"><i class="fa-solid fa-paper-plane me-2"></i> Submit Ticket</button>
                            </form>
                        </div>
                    </div>
                    ...
                    <div class="card border-0 shadow-sm mt-4">
                        <div class="card-body text-center py-4">
                            <i class="fa-brands fa-telegram text-info mb-2" style="font-size: 2rem;"></i>
                            <h6 class="fw-bold">Prefer Telegram?</h6>
                            <p class="text-muted small mb-2">Get faster support by messaging us directly.</p>
                            <a href="https://t.me/<?= htmlspecialchars(ltrim($telegram_username, '@')) ?>" target="_blank" class="btn btn-outline-info btn-sm"><i class="fa-brands fa-telegram me-1"></i> @<?= htmlspecialchars(ltrim($telegram_username, '@')) ?></a>
                        </div>
                    </div>
                </div>
                ...
                <div class="col-lg-7">
                    <div class="card border-0 shadow-sm">
                        <div class="card-header bg-transparent border-bottom py-3">
                            <h5 class="mb-0 fw-bold">My Tickets</h5>
                        </div>
                            ...
                            <div class="list-group list-group-flush">
                                            ...
                                            <div class="d-flex justify-content-between align-items-start">
                                                <div>
                                                    <div class="d-flex align-items-center gap-2 mb-1">
                                                        <?php
                                                            $statusClass = 'bg-warning text-dark'; // open
                                                            if ($t['status'] === 'in_progress') $statusClass = 'bg-info';
                                                            if ($t['status'] === 'closed') $statusClass = 'bg-success';
                                                        ?>
                                                        <span class="badge <?= $statusClass ?>"><?= ucfirst(str_replace('_', ' ', $t['status'])) ?></span>
                                                        <h6 class="mb-0 fw-bold">
                                                            <?= htmlspecialchars($t['subject']) ?>
                                                            <?php if ($t['user_read'] == 0): ?>
                                                                <span class="badge bg-danger ms-2 user-unread-badge-<?= $t['id'] ?>">New</span>
                                                            <?php endif; ?>
                                                        </h6>
                                                    </div>
                                                    <p class="text-muted small mb-1"><?= nl2br(htmlspecialchars(substr($t['message'], 0, 100))) ?><?= strlen($t['message']) > 100 ? '...' : '' ?></p>
                                                    <small class="text-muted"><i class="fa-solid fa-clock me-1"></i> <?= date('M d, Y', strtotime($t['created_at'])) ?></small>
                                                </div>
                                                 <button type="button" class="btn btn-sm btn-light btn-view-ticket" 
                                                         data-id="<?= $t['id'] ?>"
                                                         data-unread="<?= $t['user_read'] == 0 ? '1' : '0' ?>"
                                                         data-subject="<?= htmlspecialchars($t['subject']) ?>" 
                                                         data-date="<?= date('M d, Y H:i', strtotime($t['created_at'])) ?>" 
                                                         data-status="<?= htmlspecialchars($t['status']) ?>" 
                                                         data-message="<?= htmlspecialchars($t['message']) ?>" 
                                                         data-reply="<?= htmlspecialchars($t['admin_reply'] ?? '') ?>">
                                ...
                                <?php else: ?>
                                    <div class="text-center py-4 text-muted">
                                        <i class="fa-solid fa-ticket" style="font-size: 2.5rem; opacity: 0.3;"></i>
                                        <p class="mt-2 mb-0">No tickets yet. Submit one if you need help!</p>
                                    </div>
                                <?php endif; ?>
                            </div>
                        </div>
    ...
    <script>
        ...
        $(document).ready(function() {
            $('.btn-view-ticket').on('click', function() {
                ...
                const isUnread = $(this).attr('data-unread') === '1';
                const $btn = $(this);
                ...
                statusBadge.removeClass('bg-warning bg-info bg-success text-dark text-white');
                if (status === 'open') {
                    statusBadge.addClass('bg-warning text-dark').text('Open');
                } else if (status === 'in_progress') {
                    statusBadge.addClass('bg-info text-white').text('In Progress');
                } else if (status === 'closed') {
                    statusBadge.addClass('bg-success text-white').text('Resolved');
                }

                $('#modalMessage').text(message);

                // Admin reply visibility
                const replyContainer = $('#adminReplyContainer');
                if (reply && reply.trim() !== '') {
                    $('#modalAdminReply').text(reply);
                    replyContainer.show();
                } else {
                    replyContainer.hide();
                }
                ...
                if (isUnread && ticketId) {
                    fetch('api/mark_ticket_read', {
                        method: 'POST',
                        headers: {
                    ...
                    .then(data => {
                        if (data.status === 'success') {
                            $btn.attr('data-unread', '0');
                            $(`.user-unread-badge-${ticketId}`).fadeOut();
                            
                            // Decrement global sidebar badges
                            $('.user-support-badge').each(function() {
                    ...
                    .catch(err => console.error('Error marking ticket as read:', err));
                }
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content border-0 shadow-lg">
                <div class="modal-header bg-light py-3">
                    <h5 class="modal-title fw-bold" id="ticketModalLabel">Ticket Details</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body p-4">
                    <div class="mb-3">
                        <span class="text-uppercase text-2xs fw-bold text-muted d-block mb-1">Subject</span>
                        <h6 class="fw-bold text-dark mb-0" id="modalSubject">-</h6>
                    </div>
                    <div class="mb-3">
                        <span class="text-uppercase text-2xs fw-bold text-muted d-block mb-1">Date Created</span>
                        <small class="text-muted" id="modalDate">-</small>
                    </div>
                    <div class="mb-3">
                        <span class="text-uppercase text-2xs fw-bold text-muted d-block mb-1">Status</span>
                        <span class="badge" id="modalStatus">-</span>
                    </div>
                    <div class="mb-3">
                        <span class="text-uppercase text-2xs fw-bold text-muted d-block mb-1">Your Message</span>
                        <div class="bg-light rounded-3 p-3 text-dark small" id="modalMessage" style="white-space: pre-wrap;">-</div>
                    </div>
                    <div class="border-top pt-3" id="adminReplyContainer">
                        <span class="text-uppercase text-2xs fw-bold text-muted d-block mb-2">Admin Reply</span>
                        <div class="bg-primary bg-opacity-10 border border-primary border-opacity-10 rounded-3 p-3 text-dark small" id="modalAdminReply" style="white-space: pre-wrap;">-</div>
                    </div>
                </div>
