...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!verify_csrf_token($_POST['csrf_token'] ?? '')) {
        ...
        $status = in_array($_POST['status'] ?? '', ['active', 'pending']) ? $_POST['status'] : $video['status'];
        ...
        if (empty($title)) {
            ...
            $update = $pdo->prepare("UPDATE videos SET title = ?, price = ?, status = ? WHERE id = ? AND user_id = ?");
            $update->execute([$title, $price, $status, $id, $user_id]);
...
<head>
  ...
  <title><?= htmlspecialchars($platform_name) ?> - Edit Video</title>
          ...
          <ul class="menubar">
              ...
              <li class="menu-item menu-arrow">
                  <a class="menu-link" href="javascript:void(0);" role="button">
                      <i class="fa-solid fa-video"></i><span class="menu-label">Video</span>
