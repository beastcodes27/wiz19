...
// Fetch subscribers for all packages belonging to this user
$stmt = $pdo->prepare("
    SELECT ps.*, p.name AS package_name, p.price AS package_price,
           DATE_ADD(ps.created_at, INTERVAL p.duration_days DAY) AS expires_at
    FROM package_subscribers ps
...
<head>
  ...
  <title><?= htmlspecialchars($platform_name) ?> - Package Subscribers</title>
        ...
        <div class="container-fluid px-4">
            <div class="app-page-head d-flex align-items-center justify-content-between my-4">
                <div>
                    ...
                    <p class="text-muted small mb-0">See Your Package Subscribers</p>
                </div>
                            ...
                            <tbody>
                                <?php foreach($subscribers as $index => $sub): ?>
                                    <tr>
                                        ...
                                        <td><?php echo number_format($sub['package_price']); ?> TZS</td>
                                        <td><?php echo htmlspecialchars($sub['subscriber_phone']); ?></td>
                                        <td><?php echo date('d M Y, H:i', strtotime($sub['created_at'])); ?></td>
                                        <td><?php echo date('d M Y, H:i', strtotime($sub['expires_at'])); ?></td>
                                        <td>
                                            <?php
                                            $isExpired = strtotime($sub['expires_at']) < time();
                                            $statusClass = $isExpired ? 'bg-soft-danger text-danger' : 'bg-soft-success text-success';
