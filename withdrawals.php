...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['request_withdrawal'])) {
    $message = "Withdrawal requests are currently disabled. Please contact support.";
    $messageType = "danger";
}

// Fetch Withdrawal History
$historyStmt = $pdo->prepare("SELECT * FROM withdrawals WHERE user_id = ? ORDER BY created_at DESC");
$historyStmt->execute([$user_id]);
$withdrawals = $historyStmt->fetchAll();
?>
<!DOCTYPE html>
<html lang="en">
...
<head>
  ...
  <title><?= htmlspecialchars($platform_name) ?> - Earnings & Withdrawals</title>
    ...
    <main class="app-wrapper">
        <div class="container">
            <div class="app-page-head d-flex flex-wrap gap-3 align-items-center justify-content-between">
                <div class="clearfix">
                    <h1 class="app-page-title">Earnings & Withdrawals</h1>
                </div>
            </div>
                ...
                <div class="col-md-6">
                    <div class="card bg-primary text-white">
                        <div class="card-body">
                            ...
                            <h2 class="display-6 fw-bold mb-0">TZS <?php echo number_format($user['balance'] ?? 0, 2); ?></h2>
                        </div>
                    </div>
                </div>
                <div class="col-md-6">
                    <div class="card">
                        <div class="card-body">
                            ...
                            <div class="alert alert-warning border-0 shadow-sm d-flex align-items-center mb-0" role="alert">
                                ...
                                <div>
                                    Withdrawals are temporarily disabled. Please contact support.
                                </div>
                            </div>
                        </div>
                    </div>
                            ...
                            <table class="table table-striped">
                                ...
                                <tbody>
                                    <?php if (empty($withdrawals)): ?>
                                        <tr><td colspan="4" class="text-center">No withdrawals found.</td></tr>
                                    <?php else: ?>
                                        <?php foreach ($withdrawals as $w): ?>
                                        <tr>
                                            <td><?php echo date('Y-m-d H:i', strtotime($w['created_at'])); ?></td>
                                            <td>TZS <?php echo number_format($w['amount'], 2); ?></td>
